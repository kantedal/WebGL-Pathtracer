...
void getNodeData(int index, inout BVHNode node) {
  ...
  vec3 extra_data = vec3(texture(u_objects_bvh_texture, sample3));
...
void traverseObjectTree(Ray ray, inout Collision closest_collision, int start_index, int triangle_start_index) {
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    if (node.is_leaf == 0.0) {
      ...
      bool collision = BoundingBoxCollision(node.bottom_bbox, node.top_bbox, ray, collision_distance);
      if (collision) {
        // Check distance to bounding box collision
        if (collision_distance < closest_collision.distance) {
          stack[stackIdx++] = node.extra_data1; // Set left child index: extra_data1 = left index
          stack[stackIdx++] = node.extra_data2; // Set right child index: extra_data2 = right index

          // Return if stack index exceeds stack size
          if (stackIdx > 15) return;
        }
      }
    }
    else {
      ...
      for (int idx = start_triangle_index; idx < end_index; idx++) {
        Triangle triangle = GetTriangleFromIndex(getTriangleIndex(idx));
