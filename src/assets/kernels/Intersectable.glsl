...
void getObjectAtIndex(int index, inout Object object, inout int object_type) {
  vec2 sample_step = vec2(1,0) / vec2(512, 512);
  vec2 start_sample = vec2(1,0) / vec2(512, 512) * float(index) * 3.0;
  ...
  object = Object(bottom_bbox, top_bbox, bvh_start_index, triangle_start_index);
}
